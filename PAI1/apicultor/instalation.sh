...
logHashes=$HOME/.apicultor/history/hash.log
...
function write_log()
{
    #Recibe fichero y num de diferencias
    nHashes=\$(wc -l < "\$1")
    xx=\$2; yy=$nHashes; porcentaje=\$(echo "scale=4; (\$xx/\$yy*100)" | bc)
    echo "Se han producido \$porcentaje% de cambios en \$1">>\$logHashes
}
...
function main()
{
    ...
    write_log $hashesAntiguos $nCambios
    rm \$diffHashes
